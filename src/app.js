   ...
   if(req.cookies['username']!=undefined)
{
   ...
   if(pid!=undefined)
   {
      ...
      res.render("projects",{links : user.projects,members : project.members,projectname:project.projectname,projecturl:project.projecturl});
    ...
    { console.log(furl);
       ...
       for(var j = 0;  j < user.requests.length; j++) {
          if (user.requests[j].file === furl) {
            console.log(user.requests[j].pending);
      ...
      //     if (project.files[k].filename === req.cookies['file']) {
      //      var temp =  project.files[k].verified;
      //      project.files[k].verified = temp+1;
